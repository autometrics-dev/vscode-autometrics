...
export type Imports = {
  ...
  ) => Promise<types.Result<types.HttpResponse, types.HttpRequestError>>;
  now: () => types.Timestamp;
...
export type Exports = {
  createCells?: (
    queryType: string,
    response: types.Blob,
  ) => types.Result<Array<types.Cell>, types.Error>;
  extractData?: (
    response: types.Blob,
    mimeType: string,
    query: string | null,
  ) => types.Result<types.Blob, types.Error>;
  getConfigSchema?: () => types.ConfigSchema;
  getSupportedQueryTypes?: (
    config: types.ProviderConfig,
  ) => Promise<Array<types.SupportedQueryType>>;
  invoke2?: (
    request: types.ProviderRequest,
  ) => Promise<types.Result<types.Blob, types.Error>>;
...
export async function createRuntime(
  ...
  const getExport = <T>(name: string): T => {
    ...
    if (!exp) {
      throw new FPRuntimeError(
        `Plugin did not export expected symbol: "${name}"`,
      );
    }
  ...
  const resolveFuture = getExport<
    (asyncValuePtr: FatPtr, resultPtr: FatPtr) => void
  ...
  return {
    createCells: (() => {
      ...
      return (queryType: string, response: types.Blob) => {
        ...
        return parseObject<types.Result<Array<types.Cell>, types.Error>>(
          export_fn(query_type_ptr, response_ptr),
        );
      };
    })(),
    extractData: (() => {
      ...
      return (response: types.Blob, mimeType: string, query: string | null) => {
        ...
        return parseObject<types.Result<types.Blob, types.Error>>(
          export_fn(response_ptr, mime_type_ptr, query_ptr),
        );
      };
    })(),
    getConfigSchema: (() => {
    ...
    getSupportedQueryTypes: (() => {
      ...
      return (config: types.ProviderConfig) => {
        ...
        return promiseFromPtr(export_fn(config_ptr)).then((ptr) =>
          parseObject<Array<types.SupportedQueryType>>(ptr),
        );
      };
    })(),
    invoke2: (() => {
      ...
      return (request: types.ProviderRequest) => {
        ...
        return promiseFromPtr(export_fn(request_ptr)).then((ptr) =>
          parseObject<types.Result<types.Blob, types.Error>>(ptr),
        );
      };
    })(),
    createCellsRaw: (() => {
